...
- **Layered stylesheets.** `style.css` is the shared 24-page base; `motion.css`, `rails.css`, `vivid.css`, `alive.css`, `glass.css`, `dynamics.css`, `living.css` and `throughline.css` are opt-in, separately cache-busted layers, each gated on one `data-*` attribute. `theme.css` + `theme.js`, and `lexicon.css` + `receipts.css`, are site-wide (all 24) — the appearance choice and the signature underline respectively.
...
- **The signature underline** (`lexicon.css`, site-wide, all 24 pages): **one** hairline dotted underline in `--muted`, meaning exactly one thing — *there is substance under this* — with two consumers and no third. **The Lexicon** (`lexicon.js`) marks a **term**: the 126-entry `data/concepts.json` index, auto-linked in prose, first occurrence per concept per page, exact-case only (several aliases are acronyms — `OR`, `LP`, `T1` — that a case-insensitive match would fire on every sentence on the site). Its card is depth-aware: "read it in full" points at the tier that *teaches* the concept, and `tiers.js`'s existing hash-reveal opens that tier, so a 🟢 reader who taps a 🔴 term lands on the plain-language treatment. **Show the receipts** (`receipts.js`) marks a **number**: a load-bearing figure with a record in `data/receipts.json` — value, derivation, inputs, the `tools/` script or citation behind it, the date, and one *wrong if:* line. Both share one popover engine (`window.SymbiQ.sig`, owned by `lexicon.js`) — never build a second floating card. The page-level **"Show the receipts"** toggle (`receipts.css`) auto-mounts after `.wrap h1` on any page with ≥1 record and fills every marked underline at once: the frame where the page reads as an instrument readout. **Scope is the whole point** — mark the few figures an argument *rests* on, never every number; marking everything makes the marked ones mean nothing. `tools/check_receipts.py` fails the build on a receipt naming a `tools/` script that no longer exists, a `data-rcpt` id with no record, or a record whose page carries no span — a receipt that rots is worse than no receipt.
...
- **The vivid layer** (`vivid.css`, `body[data-vivid]`): an opt-in structural richening on the six landing/pillar pages (`index`, `ai`, `bitcoin`, `compare`, `logical-qubit`, `qec`). Two palette eras: dialled back to within a hair of the base on 2026-08-28, then **retinted warm on 2026-09-02, dark mode only** (Chinmoy's call — the light block is untouched, deliberately: these six pages stay dark-only in mood even though the site's explicit Light choice still works on them). Dark-mode tokens now run `--bg #16100f` (a warm umber-plum void, not blue-black), `--violet #c586e6` ("amethyst," rotated toward magenta) and `--teal #33d696` ("emerald," rotated toward green) — a deliberate move away from the cool blue-black-plus-cyan-purple palette the rest of the dark-tech web defaults to. `--indigo` (the h1/hairline spectrum's bridge stop, decorative only) is now a warm ember tone rather than cool indigo, despite the variable's inherited name. Every text-bearing token re-measured before shipping — ink/bg 15.9:1, muted/bg 7.6:1, amethyst/bg 7.1:1, emerald/bg 10:1. Ink and muted stay neutral on purpose (this retints the ground and the two brand accents, not body text). `theme.css` mirrors the same values for the explicit Dark/Dim choice. What "vivid" means STRUCTURALLY is unchanged from 2026-08-28: a larger 800-weight hero, the h1/`h2::before` spectral three-stop ramp, a top-light sheen and hue-tinted shadow on cards, and an ambient motion set (breathing background, aurora heroes, and the shared `.readbar` progress line taking the spectrum and a third pixel). `.bigplay` is untouched. Reversible by removing one attribute.
...
- **The theme layer** (`theme.css` + `theme.js`, site-wide, all 24 pages): turns the implicit "follow the OS" scheme into an explicit remembered choice — **System / Light / Dim / Dark**, plus **reduce motion** and **reduce transparency** switches, in a quiet control appended to every page's `<footer>`. A tiny inline `<head>` script applies the saved `data-theme` / `data-motion-pref` / `data-transparency-pref` on `<html>` **before first paint** (no flash); `theme.js` then builds the control, syncs `<meta name="theme-color">`, and mirrors changes across tabs. **Dim** is a new softer-dark surface tone (canvas lifts off true black, text steps back from pure white, hairlines ease) — same hues, same accents, the semantic colour law untouched. Because an attribute can't re-trigger a media query, `theme.css` **mirrors** the token sets from `style.css` / `vivid.css` / `glass.css` under `:root[data-theme=…]` selectors (specificity + load order do the overriding); style.css is not touched. The mirror must be kept in sync — the file's header lists exactly which source blocks it copies. "System" removes the attribute entirely, handing control back to `@media (prefers-color-scheme)`.

- **The dynamics layer** (`dynamics.css` + `dynamics.js`, rides `body[data-glass]`, all 24 pages): the premium-feel *response* the pages were missing — feedback motion only, no ambient (the motion law stands). The category dropdowns **arrive** (a `@starting-style` fade + 6px drop, instant on any browser without it) instead of blinking, and **open on hover** on a fine pointer at ≥720px (`dynamics.js`, 220ms close grace, click + keyboard untouched, the account menu stays click-only); the frosted header **catches the light** (`dynamics.js` writes `--dyn-x/--dyn-y` on pointer-move, a white radial rides the glass); every interactive control gets one **consistent short echo** — all state properties eased on `--dur-fast`, a `scale: .985` press dip (`scale` composes with the hover `transform`, never replaces it); the header **settles** past ~40px of scroll (`data-scrolled` on `<html>`, box-shadow + backdrop-filter only). Plus the small stuff: the nav chevron **turns** on open, the nav underline **wipes in** (a `scaleX` bar, replacing the border on the active link), menu rows **lean 3px** toward the cursor, text selection is **teal**, and the scrollbar is **thin and violet** (`:has()` keeps the page-scroller rule on glass pages only). Everything is inside `prefers-reduced-motion: no-preference` or degrades to a 1ms transition. Reversible by removing one `<link>`, one `<script>`.

- **The living layer** (`living.css` + `living.js`, `body[data-living]`, on the homepage + `bitcoin` / `qec` / `ai` / `compare` / `logical-qubit` / `quantum-mechanics` / `analog`): the layer where the science *is* the decoration. Three additive pieces, each gated on an existing hook so a page that does not opt in is byte-for-byte unchanged. **The coupling strip** (`living.js` mounts into any `[data-living-strip]` element; the homepage puts one under the "couple many of the same circuits" sentence) — a real **Kuramoto** coupled-phase-oscillator relaxation: fourteen independent oscillators pulling into one rhythm while you watch, the same object every oscillator-Ising widget on the site runs (`analog.html`, `games.js`), shown at rest. Model + constants + claims brute-forced in `tools/verify_living.py`; one settled static frame under reduced motion. **The breakout** (`.breakout` on one big figure per page) — a *contained* widen (≈40–56px past `.wrap` each side, no `vw` unit — this site has a history of `vw`-vs-scrollbar overflow and `smoke.mjs` checks 375px) with a faint violet/teal radial wash and hairline top/bottom rules, so a page has one deliberate "look here" moment; the homepage Loop figure is the first. **The flow** — on a living page the homepage Loop's flow dots also advance with scroll (`animation-timeline: scroll()`, `@supports`-gated, CSS only, reduced-motion already kills it in `style.css`), so the loop runs while you read it. Nothing bounces, nothing self-starting crosses the threshold of notice, no `transform` animation with `fill: both`. Reversible by removing one attribute, one `<link>`, one `<script>`.

- **The glass layer** (`glass.css`, `body[data-glass]`, all 24 pages — shipped on the six `data-vivid` pages 2026-08-30, extended site-wide the same day): a spatial-glass *re-surface*, not a re-theme. A short list of chrome surfaces — the sticky header, the dropdown/account/mobile menus, `.card` / `.introute-card`, `.orhero`, `.gate`, and the two HUD cards — stop being flat opaque panels and read as frosted panes floating over the field: a translucent fill with `backdrop-filter: blur() saturate()`, a 1px top rim light, a hue-tinted (`--indigo` on vivid pages, plain `--border` elsewhere) hairline, and a wide soft downward depth shadow. **No text colour, no contrast-locked token, and `.bigplay` all untouched**; only background, border, box-shadow and backdrop-filter move. **No keyframe animation** — glass here is a static material, so there is nothing for reduced-motion to disable. Translucency lives inside `@supports (backdrop-filter)` with an opaque value declared first, and `@media (prefers-reduced-transparency: reduce)` forces the whole set back to solid. Reversible by removing one attribute and one `<link>`.

- **The Contradiction card** (`contradiction.css` + `contradiction.js`, on `compare` / `analog` / `race`): The Ledger, inline, at the paragraph making the claim. A page author drops one placeholder — `<div class="contra" data-claim="<slug>">` — beside a contested sentence, and the card fills in from `data/claims/<slug>.json`: the speaker, their verbatim quote and its date, every **dated rebuttal on the record** with its own source link, and the resolution date with its grace period. It converts the Ledger from a destination into something you *bump into*, which is how trust actually accumulates. **It is a second VIEW, never a second copy** — nothing is summarised into the HTML, because `race.html` carried the same wrong sentence in three hand-copied places for three days (see the Corrections page). One record, N renderings, zero copies; the cost is that a crawler sees the placeholder's static fallback link instead of the card, which is the right trade for a card whose whole value is being current. **Colour Law:** muted scaffolding by default; the red rule and kicker appear only when the ledger says so — `is-contested` (a named party published a rebuttal) or `is-overdue` (nobody disputed it, the deadline simply passed). Two classes, because they are not the same finding. **Placement rule:** never inside a tier-gated section — `tiers.js` hides the non-active tier, so a card in a 🔴 block is invisible to the default 🟢 reader. `tools/verify_contradiction.mjs` measures each card's rendered height to enforce that; `tools/check_contradictions.py` enforces that every slug resolves to a real ledger record.

- **The Through-Line** (`throughline.css` + `throughline.js`, `body[data-throughline]`, **`qec.html` only, on purpose**): one small SVG companion — a single surface-code patch — fixed in the bottom-right corner, whose state is driven by whichever `[data-fig-state]` section is in view (`IntersectionObserver` on a thin band near viewport centre). The reader accumulates *one object* across five sections (a lone qubit failing → the patch forming → a syndrome firing and a repair → calm below threshold → the same patch labelled) instead of restarting at each section's own diagram. It is a **stylised 3×3 companion, never the page's referee-verified widget** — it borrows that widget's colour vocabulary so it reads as the same object, and asserts no number of its own. **Two shapes, one figure.** ≥720px it is that fixed corner card. **Under 720px it is a sticky band** — figure, caption, dismiss, laid out sideways, in the flow just after the tagline, scrolling up with the reading and pinning to `top: 0`. The phone gets a *different object, not a smaller one*: the original objection (a fixed corner panel competes with a phone's screen for the attention the prose needs) was about a shrunken card and still stands. Two placement facts decided the rest, both measured rather than assumed — below 720px this site's header is **not** sticky, so `top: 0` is the honest pin; and the bottom edge was already taken (`alive.css`'s `.sq-marker` owns bottom-left, `#qz-pill` bottom-right). The panel mounts **into the flow**, which is what makes sticky possible; desktop is unaffected because `position: fixed` ignores the DOM slot and `.wrap` carries no transform, filter or contain. Dismissible, remembered per tab (`sessionStorage`) — a decorative companion is not worth nagging a returning reader about. **Shipped on one page deliberately**, per its own plan's "ship it, judge it, then decide"; extending it to the other pillar pages, or building the mobile variant, is a product call, not a backlog item.
