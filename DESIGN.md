...
- **The dynamics layer** (`dynamics.css` + `dynamics.js`, rides `body[data-glass]`, same six pages): the premium-feel *response* the glass pages were missing — feedback motion only, no ambient (the motion law stands). Four moments: the category dropdowns **arrive** (a `@starting-style` fade + 6px drop, instant on any browser without it) instead of blinking; the frosted header **catches the light** (`dynamics.js` writes `--dyn-x/--dyn-y` on pointer-move, a 6%-white radial rides the glass); every interactive control on the page gets one **consistent short echo** — all state properties eased on `--dur-fast`, a `scale: .985` press dip (`scale` composes with the hover `transform`, never replaces it); and the header **settles** — past ~40px of scroll `dynamics.js` sets `data-scrolled` on `<html>` and the cast deepens and the blur tightens (box-shadow + backdrop-filter only, nothing near the width budget). Everything is inside `prefers-reduced-motion: no-preference` or degrades to a 1ms transition. Reversible by removing one `<link>`, one `<script>`.
