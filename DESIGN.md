...
- **Layered stylesheets.** `style.css` is the shared 24-page base; `motion.css`, `rails.css`, `vivid.css`, `alive.css`, `glass.css` and `dynamics.css` are opt-in, separately cache-busted layers.
